...
public class FileUploadController {
    ...
    @RequestMapping(value = "/businessUser/upload", method = RequestMethod.POST)
    ...
    @RequestMapping(value = "/buyerUser/upload", method = RequestMethod.POST)
    public String buyerFileUpload(@RequestParam("avatar") CommonsMultipartFile avatar , HttpServletResponse response, HttpServletRequest request) throws IOException {
        String path=request.getServletContext().getRealPath("/upload");
        //获取文件名 : file.getOriginalFilename();
        String uploadFileName;
        uploadFileName = avatar.getOriginalFilename();
        //如果文件名为空，直接回到首页！
        if ("".equals(uploadFileName)){
